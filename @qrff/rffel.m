function [T] = rffel(element,a,w,dist)
...
%	matlab4 version. -no more

% Version upgrade: A. & Y. Greenhut

if nargin==0 ,
   disp('  [T] = rffel(element,a,w,dist)')
   return;
end;
...
if strcmp(element,'int'),            % t=1/(s^n)  n== integer; 
   ...
   T= round(-90*n/dist)*dist + j*20*log10(abs((1)./s.^n));
   T=[T;T];
end;
