function [T] = rffpz(a,w,form,pzf,dist) 
...
%
%       a: 	When the factor is uncertain, a is a vector with two 
%           real elements in the form a=[min max] 
%           min and max represent the minimum and maximum values of the
%           uncertain parameter. When the factor is certain, either min=max,
%           or a contains one element only.
%	
...
if nargin==0 ,
   disp('  [T] = rffpz(a,w,form,pzf,dist)')
   return;
end;
...
% Defaults
  ...
  if length(form) == 0 , form='dc'; end;
  if length(pzf) == 0 ,  pzf='z';   end;
...
% Round to the desired distance.  %Note: phi in (0,180) deg
  ...
  if strcmp(form,'dc') ,     
    T=c2n(j*(w./wc)+1, 180);	%Note: real(T) in (-90,90] but not nec contiguous
...
% phase rounding  
  if phi(1) > phi(2)
     T(2) = real(T(2))+j*imag(T(1));
  end;
  if phi(n) < phi(n-1)
     T(n-1) = real(T(n-1))+j*imag(T(n));
  end;
  T = T(2:n-1);
