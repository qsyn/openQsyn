...
for ii = 1:(length(num_coef))
    ...
    if (ii == length(num_coef))
        str_fraction = sprintf('%s %.5s',str_fraction, string(num_coef(ii)));  % Last term is 1
    else 
        if (num_coef(ii) == 1)
            str_fraction = sprintf('%s %.5s',str_fraction,string(num_term(ii)));
        else
            str_fraction = sprintf('%s %.5s%.5s',str_fraction,string(num_coef(ii)),string(num_term(ii)));
        end    
    end
end
str_fraction = sprintf('%s) \n ----------------------------------------------- \n ',str_fraction);
for ii = 1:(length(den_coef))
    ...
    if (ii == length(den_coef))
        str_fraction = sprintf('%s %.5s)',str_fraction, string(den_coef(ii)));  % Last term is 1
    else 
        if (den_coef(ii) == 1)
            str_fraction = sprintf('%s %.5s',str_fraction,string(den_term(ii)));
        else
            str_fraction = sprintf('%s %.5s%.5s',str_fraction,string(den_coef(ii)),string(den_term(ii)));
        end    
    end
end
