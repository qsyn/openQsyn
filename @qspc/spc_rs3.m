function [spec_w,spec_t,tab]=spc_rs3(spc_tab,w,dt,plt,n)
...
% 
%        1-alpha*exp(-b*t)-beta*exp(-p*t)*sin(ws*t+Phi)
% 
...
%        lower bound.
% 
%        See also: RSRS, ODSRS, IDSRS, SPC_RS2, SPC_RS31.
% 
...
if nargin==0
  disp('[spec_w,spec_t,tab]=spc_rs3(spc_tab,w,dt,plt,n)')
  return
end;
if ~(exist('plt')==1),
  plt=1;
...
xmax=max(ts);
ymin=min(t(stu>stu(1)));
ymax=max(ts);
qmin=[xmin,ymin,-1.5,-1.5];
qmax=[2*xmax,2*ymax,1,1];
Q=qgrid(n,qmin,qmax);
...
I=(z>=sqrt(1/(1+(pi/log(max(min(max(stu),2-sqrt(eps))-1,eps)))^2)))&(b>w0/3)&(a>0);
z=z(I);
a=a(I);
w0=w0(I);
xi=xi(I);
...
for j=floor(log2(length(T))):(-1):0
  ...
  for k=1:(2^j):length(T)
    st=1-((1-eta).*exp(-b*T(k))+exp(-p*T(k)).*(A.*cos(ws*T(k))+...
      B.*sin(ws*T(k))))./C;
    I=(st<=STU(k))&(st>=STL(k));
    if plt
...
%no_inside=0;
for k=1:length(a)
  st=1-((1-eta(k)).*exp(-b(k)*t)+exp(-p(k)*t).*(A(k).*cos(ws(k)*t)+...
    B(k)*sin(ws(k)*t)))/C(k);
  mag=20*log10(abs((i*w/a(k)+1)./(i*w/b(k)+1)./(1-w.^2/w0(k)^2+...
    i*2*z(k)*w/w0(k))));
